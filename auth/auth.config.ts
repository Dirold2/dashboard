...
export default {
  providers: [
    ...
    Credentials({
      ...
      authorize: async (credentials) => {
        ...
        const passwordsMatch = crypto.createHash('sha256')
                                    .update(password)
                                    .digest('hex');

        user = await prisma.user.findUnique({ where: { name: name, password: passwordsMatch } });
  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl }): Promise<string> {
      ...
      if (url.startsWith(`/${locale}/login`)) return `${baseUrl}/${locale}/account`;
